  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Buscar pontuação e classe
        const pontos = await api.get("/treino-dia/pontuacao");
        setPontuacao(pontos.data.pontos_mes);
        setClasseAtual(pontos.data.classe);
        setDiasRestantes(pontos.data.dias_restantes);
      } catch (error) {
  ...
  useEffect(() => {
    async function buscarProgresso() {
      try {
        ...
        inicio.setDate(inicio.getDate() - hoje.getDay() + 1);
      ...
      >
        ...
        <ScrollView contentContainerStyle={homeStyles.content}>
          ...
          <View style={homeStyles.card}>
            ...
            <View style={homeStyles.progressoContainer}>
              <Text style={homeStyles.progressoTexto}>📊 Progresso da semana</Text>
              {progressoSemanal > 0 && (
                <View style={homeStyles.barraContainer}>
                  <View
                    style={[
                      ...
                      {
                        width: `${Math.min(progressoSemanal || 0, 100)}%`,
            ...
            <Text style={[homeStyles.subtitulo, { textAlign: "center", marginTop: 6 }]}>🏆 Classe atual: {classeAtual}</Text>
            {diasRestantes > 0 && (
              <Text style={[homeStyles.subtitulo, { textAlign: "center", fontSize: 12 }]}>Faltam {diasRestantes} dia(s) para avançar</Text>
            )}
          </View>
